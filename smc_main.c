...
static void smc_init_leds(struct smc_data *data)
{
	// initializing leds
	int ret = 0;
	char name[VEC6200_SMC_LED_MAX_NAME_LEN + 1];
...
static struct i2c_device_id smc_id[] = { { "vec6200-mcu", 0 }, { } };

MODULE_DEVICE_TABLE( i2c, smc_id);

static struct i2c_driver ct_smc_driver = { .driver = {
	.owner = THIS_MODULE, .name = "vec6200_mcu", .of_match_table =
...
MODULE_DESCRIPTION("VEC6200 System Management Controller");
MODULE_LICENSE("GPL");
